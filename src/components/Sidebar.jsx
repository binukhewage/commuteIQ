...
export default function Sidebar() {
  ...
  return (
    <>
      ...
      {/* Mobile Top Header */}
      <div className="md:hidden fixed top-0 left-0 right-0 bg-background/80 backdrop-blur-2xl border-b border-border flex items-center justify-between px-4 py-2 pt-[calc(env(safe-area-inset-top)+0.5rem)] z-50">
        <div className="flex items-center gap-2">
          <div className="p-1.5 rounded-lg bg-gradient-to-tr from-indigo-600 to-cyan-500 text-white shadow shadow-indigo-500/20">
            <Car className="size-4" />
          </div>
          <span className="font-bold text-sm tracking-tight bg-clip-text text-transparent bg-gradient-to-r from-foreground to-muted-foreground">CommuteIQ</span>
        </div>
      ...
      <nav className="md:hidden fixed bottom-0 left-0 right-0 bg-sidebar/85 backdrop-blur-2xl border-t border-sidebar-border flex items-center justify-around px-2 pb-[calc(env(safe-area-inset-bottom)+0.5rem)] pt-2 z-50 shadow-[0_-8px_30px_rgb(0,0,0,0.12)]">
        {navItems.map((item) => {
          const isActive = pathname === item.href;
          const Icon = item.icon;
          ...
          return (
            <Link
              ...
              href={item.href}
              className={cn(
                "flex flex-col items-center justify-center gap-1 flex-1 py-2 px-1 rounded-xl transition-all relative overflow-hidden",
                isActive ? "text-indigo-500" : "text-sidebar-foreground/50 hover:text-sidebar-foreground"
              )}
            >
              <div className={cn(
                "absolute inset-0 bg-indigo-500/10 rounded-xl transition-opacity duration-300",
                isActive ? "opacity-100" : "opacity-0"
              )} />
              <Icon className={cn("size-5 relative z-10 transition-transform duration-300", isActive ? "-translate-y-0.5" : "")} />
              <span className={cn("text-[10px] tracking-wide font-bold relative z-10 transition-transform duration-300", isActive ? "translate-y-0.5" : "")}>{item.name}</span>
            </Link>
          );
        })}
      </nav>
